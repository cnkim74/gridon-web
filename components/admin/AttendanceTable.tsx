    ...
    <>
      <div className="apage-head">
        ...
        <div className="flex gap-s wrap">
          <button className="btn btn--sm" onClick={() => setEditing("new")} disabled={!empId}>＋ 기록 추가</button>
        </div>
      </div>
      ...
      <div className="panel">
        <table className="dtable">
          ...
          <tbody>
            {!empId && <tr><td colSpan={6} className="cellsub" style={{ textAlign: "center", padding: 28 }}>직원을 먼저 등록하세요 (직원 현황).</td></tr>}
            {empId && loading && <tr><td colSpan={6} className="cellsub" style={{ textAlign: "center", padding: 28 }}>불러오는 중…</td></tr>}
            {empId && !loading && err && <tr><td colSpan={6} style={{ textAlign: "center", padding: 28, color: "#b3261e" }}>오류: {err}</td></tr>}
            {empId && !loading && !err && recs!.length === 0 && <tr><td colSpan={6} className="cellsub" style={{ textAlign: "center", padding: 28 }}>{month} 기록이 없습니다. “기록 추가”로 입력하세요.</td></tr>}
            {empId && !loading && !err && recs!.map((r) => (
              <tr key={r.id}>
...
function AttendanceModal({
  employeeId, employeeName, record, onClose, onSaved,
}: {
  employeeId: string;
  employeeName: string;
  record: Rec | null;
  ...
  const [date, setDate] = useState(record?.work_date ?? today());
